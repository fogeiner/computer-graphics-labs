...
public class Scene extends JPanel {
	...
	class MouseHandler extends MouseAdapter implements MouseMotionListener,
		...
		public void mouseWheelMoved(MouseWheelEvent e) {
			...
			Transformation t = Transformation.translate(0, 0, wheelRotationSign
					* rollCoef);
			transformAllObjects(t);
			repaint();
		}
	...
	private void initBoundBox() {
		...
		double boxX = (maxX + minX) / 2, boxY = (maxY + minY) / 2, boxZ = (maxZ + minZ) / 2;

		savedBoxOrigin = new Vertex(boxX, boxY, boxZ);

		box = WireframeShape.parallelepiped(maxX - minX, maxY - minY, maxZ
				- minZ);
	...
	private void initOrts() {
		...
		WireframeShape x = WireframeShape.segment(ORT_COEF * rect.getWidth()
				/ 2, 0, 0), y = WireframeShape.segment(0,
				ORT_COEF * rect.getHeight() / 2, 0), z = WireframeShape
				.segment(0, 0, ORT_COEF * rect.getDepth() / 2);
	...
	private void initCamera() {
		...
		// Camera is fixed, aimed against z axis and in world coordinates
		// (0,0,0)
		Vertex boxOrigin = box.getTransformedOrigin();

		Transformation translate = Transformation.translate(-boxOrigin.getX(),
				-boxOrigin.getY(), -boxOrigin.getZ());
		...
		double cameraX = 0, cameraY = 0, cameraZ = d + boxSizes.getDepth() / 2
				+ 1;

		translate = Transformation.translate(-cameraX, -cameraY, -cameraZ);
	...
	protected void paintComponent(Graphics g1) {
		...
		for (WireframeShape shape : getAllObjects()) {
			...
			for (Segment s : shape.getTransformedSegments()) {
				...
				double sx = startProjected.getX(), sy = startProjected.getY(), sz = startProjected
						.getZ(), ex = endProjected.getX(), ey = endProjected
						.getY(), ez = endProjected.getZ();
	...
	public void init() {
		...
		box.transform(Transformation.translate(savedBoxOrigin.getX(),
				savedBoxOrigin.getY(), savedBoxOrigin.getZ()));
		initCamera();
		zfar = savedZfar;
		znear = savedZnear;
		repaint();
