...
public class Filters {
	...
	public static BufferedImage getSobelImage(BufferedImage o, double threshold) {
		...
		for (int h = 0; h < height - 2; ++h) {
			for (int w = 0; w < width - 2; ++w) {
				...
				g23 = data[(h + 2) * width + (w + 1)] & 0xFF, g31 = data[h
						* width + (w + 2)] & 0xFF, g32 = data[(h + 1) * width
						+ (w + 2)] & 0xFF, g33 = data[(h + 2) * width + (w + 2)] & 0xFF;
	...
	public static BufferedImage getFloydSteinbergDitheredImage(BufferedImage o,
		...
		double errors[][][] = new double[][][] { new double[width][height],
				new double[width][height], new double[width][height] };
		int gradations[][] = { Filters.generateGradations(255, r_count),
		...
		for (int h = 0; h < height; ++h) {
			for (int w = 0; w < width; ++w) {
				...
				for (int k = 0; k < 3; ++k) {
					...
					int closest = Filters.findClosestInGradation(RGB[k],
							gradations[k]);
