...
public class FltFrame extends MainFrame implements FltFrameService {
	...
	public FltFrame(int width, int height) {
		...
		try {
			...
			addMenuItem("Edit/From B to C", "Copy image from zone B to C",
					KeyEvent.VK_Z, "next.gif", "onFromBtoC");

			addMenuItem("Edit/From C to B", "Copy image from zone C to zone B",
			...
			addToolBarButton("Edit/From B to C");
	...
	class FloydSteinbergDitheringDialog extends JDialog {
		...
		private JSpinner r = new JSpinner(new SpinnerNumberModel(8, 2, 256, 1));
		private JSpinner g = new JSpinner(new SpinnerNumberModel(8, 2, 256, 1));
		private JSpinner b = new JSpinner(new SpinnerNumberModel(4, 2, 256, 1));
	...
	class SobelDialog extends SliderSpinnerDialog {
		 ...
		 */
		public SobelDialog() {
			super("Sobel threshold dialog", 135, 0, 255 * 6);
		}
	}
	...
	class RobertsDialog extends SliderSpinnerDialog {
		...
		public RobertsDialog() {
			super("Roberts threshold dialog", 35, 0, 510);
		}
	}
	...
	/**
	 * Method is invoked after user presses From C to B toolbar buttons or
	 * chooses Edit -> From C to B
	 */
	...
	/**
	 * Method is invoked after user presses From B to C toolbar buttons or
	 * chooses Edit -> From B to C
	 */
	public void onFromBtoC() {
		BufferedImage img = _zone_b.getImage();
		_zone_c.setImage(img.getSubimage(0, 0, img.getWidth(), img.getHeight()));
	}
	...
	public void setFiltersBlocked(boolean value) {
		...
		for (int i = 8; i < 28; ++i) {
			if (i != 10 && i != 13 && i != 15 && i != 18 && i != 21 && i != 24) {
	...
	public void setFromCtoBBlocked(boolean value) {
		JMenuBar menu_bar = getJMenuBar();
		JMenu edit = (JMenu) menu_bar.getComponent(1);
		edit.getMenuComponent(2).setEnabled(!value);
		toolBar.getComponent(6).setEnabled(!value);
	}
