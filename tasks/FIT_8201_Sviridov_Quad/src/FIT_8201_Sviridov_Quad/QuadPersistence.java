...
public class QuadPersistence {
    ...
    public static Model loadFromFile(File file) throws FileNotFoundException, IOException {
        ...
        // 3) ntree – глубина дерева трассировки. Здесь значение 1. Если другое, то считать = 1
        str = LineParseUtils.nextNormalizedLine(bufferedRedeader);
        int treeDepth = Integer.parseInt(str);
        if (treeDepth != 1) {
            treeDepth = 1;
        }
        ...
        Color ambientColor = new Color((float) ds[0], (float) ds[1], (float) ds[2]);
        ...
        for (int i = 0; i < lightSourcesCount; ++i) {
            ...
            strs = LineParseUtils.nextNormalizedLine(bufferedRedeader).split(" ");
            ds = strsToDoubles(strs, 3);
            Color lightSourceColor = new Color((float) ds[0], (float) ds[1], (float) ds[2]);
        }
        ...
        for (int i = 0; i < objectsCount; ++i) {
            SceneObject sceneObject = parseSceneObject(bufferedRedeader);
            if (sceneObject != null) {
                // add to the model
            }
        }
    ...
    private static SceneObject parseSceneObject(BufferedReader bufferedReader) throws IOException {
        String strs[] = null;
        String str = null;
        double ds[] = null;
